...
namespace NodeGLPK {
    ...
    public:
        ...
        static Local<Value> Instantiate(glp_tree* tree){
            Nan::MaybeLocal<Function> cons = Nan::GetFunction(Nan::New<FunctionTemplate>(constructor));
            Local<Value> ret = cons.ToLocalChecked();
    ...
    private:
        ...
        static NAN_METHOD(RowAttrib) {
            ...
            glp_attr attr;
            GLP_CATCH_RET(glp_ios_row_attr(host->handle, GLP_TO_INT32(info[0]), &attr);)
        ...
        static NAN_METHOD(AddRow) {
            ...
            for (unsigned int i = 0; i < count; i++){
                pind[i] = GLP_TO_INT32(ind);
                pval[i] = GLP_TO_NUMBERVALUE(val);
            }
            
            count--;
            GLP_CATCH(info.GetReturnValue().Set(glp_ios_add_row(tree->handle, V8TOCSTRING(info[0]), GLP_TO_INT32(info[1]),
                GLP_TO_INT32(info[2]), count, pind, pval, GLP_TO_INT32(info[5]), GLP_TO_NUMBERVALUE(info[6])));)
        ...
        static NAN_METHOD(HeurSol) {
            ...
            for (int i = 0; i < count; i++) px[i] = GLP_TO_NUMBERVALUE(x);
